...
const bob = getAccount(mockup_mode ? 'bob' : "quartz_whitelister");
const carl = getAccount(mockup_mode ? 'carl' : "quartz_minter");
...
describe("Deploy & init", async () => {
  it.only("Whitelist users storage", async () => {
    [whitelist_users, _] = await deploy('../fa2.arl', {
      parameters: {
        admin: admin.pkh
      },
      named: 'bookitNFT',
